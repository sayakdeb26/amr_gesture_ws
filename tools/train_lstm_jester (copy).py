...
# Dataset
# -----------------------------
class BalancedNpzWindows(Dataset):
...
class OptimizedLSTMClassifier(nn.Module):
    def __init__(self, in_dim=84, hidden=128, num_classes=12, num_layers=2, dropout=0.3):
        super().__init__()
...
def train_one_epoch(model, dl, opt, loss_fn, device, grad_clip=1.0):
    ...
    return correct / total, total_loss / len(dl)

@torch.no_grad()
def eval_model(model, dl, device, labels):
    model.eval()
    total = 0
    correct = 0
    all_preds = []
...
class FlatWrap(nn.Module):
    """Wraps the LSTM classifier to accept flat 2520-dim input."""
...
def main():
    ...
    train_ds = BalancedNpzWindows(train_dir, augment=True)
    val_ds   = BalancedNpzWindows(val_dir, augment=False)
    ...
    # Write labels.txt for ROS node
    labels_path = os.path.join(args.out_dir, "labels.txt")
    with open(labels_path, "w") as f:
        f.write("\n".join(labels))
    print(f"labels.txt written to: {labels_path}")

    print("\n🚀 Starting optimized training...")
    for ep in range(1, args.epochs + 1):
        train_acc, train_loss = train_one_epoch(
            model, train_dl, opt, loss_fn, device, args.grad_clip
        )
        val_acc = eval_model(model, val_dl, device, labels)
        ...
        print(
            f"[epoch {ep:3d}] train_acc={train_acc:.4f} "
            f"train_loss={train_loss:.4f} val_acc={val_acc:.4f} lr={new_lr:.2e}"
        )
    ...
    print(f"\n✅ Training completed! Best val_acc={best_val_acc:.4f} at epoch {best_epoch}")

    # -----------------------------
    # ONNX export from best model (CPU)
    # -----------------------------
    print("\nLoading best model for ONNX export...")
    ckpt_path = os.path.join(args.out_dir, "best_model.pth")
    ckpt = torch.load(ckpt_path, map_location="cpu")

    model_cpu = OptimizedLSTMClassifier(
        in_dim=84,
        hidden=args.hidden,
        num_classes=args.num_classes,
        num_layers=args.num_layers,
    ...
    print(f"Exporting epoch {ckpt['epoch']} with val_acc={ckpt['val_acc']:.4f}")
    ...
    print(f"\n🎯 All artifacts saved in: {args.out_dir}")
    print("   - best_model.pth")
    print("   - model_30x84.onnx")
    print("   - model_2520.onnx")
    print("   - labels.txt")
