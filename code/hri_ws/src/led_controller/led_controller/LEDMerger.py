...
class LEDMerger(Node):
    ...
    def direction_led_callback(self, msg: Int32MultiArray):
        ...
        stride = msg.layout.dim[0].stride  # 5
        for i in range(0, len(msg.data), stride):
            led_id = msg.data[i]
            r = msg.data[i+1]
            g = msg.data[i+2]
            b = msg.data[i+3]
            brightness = msg.data[i+4]
            start_index = led_id * stride
            self.person_leds[start_index:start_index+5] = [led_id, r, g, b, brightness] 
...
#new version with only sending active LEDs
    def merge_leds(self):
        # Merging-Logik wie vorher
        merged_leds_local = [0] * (self.num_leds * 5)
        for i in range(self.num_leds):
            merged_leds_local[i * 5] = i
            merged_leds_local[i * 5 + 1] = 0
            merged_leds_local[i * 5 + 2] = 0
            merged_leds_local[i * 5 + 3] = 0
            merged_leds_local[i * 5 + 4] = 0

        # Priorität: stopping > person > direction
        for i in range(0, len(merged_leds_local), 5):
            if self.stopping_leds[i+1] != 0 or self.stopping_leds[i+2] != 0 or self.stopping_leds[i+3] != 0:
                ...
                merged_leds_local[i:i+5] = self.person_leds[i:i+5]
            elif self.direction_leds[i+1] != 0 or self.direction_leds[i+2] != 0 or self.direction_leds[i+3] != 0:
                merged_leds_local[i:i+5] = self.direction_leds[i:i+5]

        self.merged_leds = merged_leds_local

        # 🔹 Nur aktive LEDs senden
        active_leds = []
        for i in range(0, len(self.merged_leds), 5):
            r, g, b = self.merged_leds[i+1], self.merged_leds[i+2], self.merged_leds[i+3]
            if r != 0 or g != 0 or b != 0:
                active_leds.extend(self.merged_leds[i:i+5])

        # Nachricht erstellen
        led_msg = Int32MultiArray()
        led_msg.layout.dim = []  # keine Dim nötig
        led_msg.layout.data_offset = 0
        led_msg.data = active_leds

        self.led_publisher.publish(led_msg)
        self.get_logger().info(f'Merged LED data published ({len(active_leds)//5} LEDs): {led_msg.data}')



# #new version with splitting message
#     def merge_leds(self):
...
#         # ---- CHUNK PUBLISHING ----
#         MAX_INTS = 100  # under 127 int32 limit
#         for start in range(0, len(self.merged_leds), MAX_INTS):
#             chunk = self.merged_leds[start:start + MAX_INTS]
#             led_msg = Int32MultiArray()
#             led_msg.layout.dim = []
#             led_msg.layout.data_offset = 0
#             led_msg.data = chunk
...
#         self.get_logger().info(f'Merged LED data published in chunks of {MAX_INTS} ints')
